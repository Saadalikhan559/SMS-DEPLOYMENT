...
//   // Get all fee items from all months
//   const getAllFees = () => {
//     const fees = [];
//     const feesSubmitted = feeSummary.fees_submitted || {};
//     Object.keys(feesSubmitted).forEach(month => {
//       if (Array.isArray(feesSubmitted[month])) {
//         fees.push(...feesSubmitted[month]);
//       }
//     });
//     return fees;
//   };

//   // SUM amounts for matching fee type pattern
//   const getTotal = (feeTypePattern) => {
//     const allFees = getAllFees();
//     return allFees
//       .filter((f) => f.fee_type && f.fee_type.includes(feeTypePattern))
//       .reduce((sum, f) => sum + Number(f.amount_paid || 0), 0);
//   };
...
//   // Get all fee types and their totals for display
//   const getFeeTypeTotals = () => {
//     const allFees = getAllFees();
//     const feeMap = new Map();
    
//     allFees.forEach(fee => {
//       const key = fee.fee_type || "Unknown";
//       if (feeMap.has(key)) {
//         feeMap.set(key, feeMap.get(key) + Number(fee.amount_paid || 0));
//       } else {
//         feeMap.set(key, Number(fee.amount_paid || 0));
//       }
//     });
...
//   const feeTotals = getFeeTypeTotals();
...
//           <tbody>
//             {/* Dynamic rows based on actual fee types */}
//             {Array.from(feeTotals.entries()).map(([feeType, amount]) => (
//               <tr key={feeType}>
//                 <td className="border border-gray-400 p-2 text-left text-black">
//                   {feeType}
//                 </td>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      >
        ...
        {/* Table with Month in Particulars */}
        <table className="w-full text-sm border border-gray-400">
          <thead className="bg-gray-100">
            <tr>
              <th className="border border-gray-400 p-2 text-left text-black">
            ...
            {feesWithMonth.map((fee, index) => {
              ...
              return (
                <tr key={`${fee.fee_type}-${index}`}>
                  ...
                  <td className="border border-gray-400 p-2 text-right text-black">
                    {Number(fee.amount_paid).toFixed(2)}/-
                  </td>
                </tr>
              );
            })}
