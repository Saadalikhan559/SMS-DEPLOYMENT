...
//     // Iterate through all annual fees
//     annualFees.forEach((fee) => {
//       if (selectedFeeIds.includes(fee.fee_id)) {
...
//     // ONLY calculate for selected months
//     const filteredGroups = getMonthlyGroups(filteredAvailableFees);
...
//   // Monthly groups for display (only filtered)
//   const monthlyGroups = useMemo(() => getMonthlyGroups(filteredAvailableFees), [filteredAvailableFees]);
...
//   // Auto-select all monthly fees - only for selected months
//   useEffect(() => {
//     if (monthlyGroups.length > 0 && Object.keys(selectedGroups).length === 0) {
//       const newGroups = {};
//       monthlyGroups.forEach((group) => {
...
//   // Initialize annual fees collect amounts
//   useEffect(() => {
...
//       });
//     }
//   }, [selectedFeeIds, selectedGroups, annualFees, monthlyGroups, annualCollectAmounts, setValue]);

//   // ----- Submit with advance payment support -----
//   const onSubmit = async (data) => {
//     const annualSelected = annualFees.filter((fee) =>
//       selectedFeeIds.includes(fee.fee_id)
//     );
...
//     await processPayment(paymentMode, chequeNumber, schoolYearId, paidAmount, annualSelected, monthlySelected, data);
//   };

//   // Process payment
//   const processPayment = async (paymentMode, chequeNumber, schoolYearId, paidAmount, annualSelected, monthlySelected, data, advanceAdjustment = null) => {
//     const fees = [];

//     annualSelected.forEach((fee) => {
//       const amount = parseFloat(annualCollectAmounts[fee.fee_id]) || 0;
//       if (amount > 0) {
//         fees.push({
//           fee_type_id: fee.fee_id,
//           month: null,
...
//   // ----- FIXED: Auto-set default amount but allow manual override -----
//   useEffect(() => {
//     const total = calculateTotalAmount();
//     if (!isNaN(total) && isFinite(total)) {
//       // Only set if user hasn't manually entered anything
...
//   // ----- Handle manual paid amount change -----
...
//     // Check if paid amount is less than total payable
//     if (paidAmount < totalPayable && paidAmount > 0) {
//       alert(`You are paying ₹${paidAmount.toFixed(2)} which is less than the total payable amount of ₹${totalPayable.toFixed(2)}. Please enter the full amount or adjust the fees.`);
//       return;
//     }
...
//                   Short by: ₹{(totalAmount - parseFloat(watchedPaidAmount)).toFixed(2)}
//                 </div>
//               )}
//             </div>
...
export const AdmissionFees = () => {
  ...
  const calculateTotalAmount = () => {
    ...
    // FIXED: Calculate monthly total from monthlyGroups
    // monthlyGroups is already filtered by selectedMonths
    monthlyGroups.forEach((group) => {
      const sel = selectedGroups[group.id];
      if (sel && sel.checked) {
        const amount = parseFloat(sel.collectAmount) || 0;
        monthlyTotal += amount;
      }
    });
    ...
    console.log("🔍 Calculation Debug:", {
      ...
      selectedGroupsKeys: Object.keys(selectedGroups)
    });
  ...
  const monthlyGroups = useMemo(() => {
    const groups = getMonthlyGroups(filteredAvailableFees);
    console.log("📊 Monthly Groups:", groups);
    return groups;
  }, [filteredAvailableFees]);
  ...
  // FIXED: Auto-select all monthly fees - only for selected months
  useEffect(() => {
    if (monthlyGroups.length > 0) {
      const newGroups = {};
      monthlyGroups.forEach((group) => {
        const due = parseFloat(group.fee.due_amount) || 0;
        // Only set if not already in selectedGroups
      ...
      // Only update if there are new groups to add
